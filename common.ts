...
import { readableStreamToString, HiBobError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new HiBobError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new HiBobError(e, e.response?.data)
                }
                if (innerError instanceof HiBobError) {
                    // Got "HiBobError" from the above try block
                    throw innerError;
                }
